...
function Write-LauncherMessage {
    ...
    Write-Host "[$stamp] [Start_Mason_CoreOnly] [$Level] $Message"
}
...
$logPath = Join-Path $launcherDir ("coreonly_{0}.log" -f $timestamp)
$markerPath = Join-Path $launcherDir 'last_coreonly.json'
...
try {
    ...
    Write-LauncherMessage 'Invoking Start_Mason2.ps1 -CoreOnly'

    & (Join-Path $repoRoot 'Start_Mason2.ps1') -CoreOnly
    ...
    $errorLine = "[{0}] [Start_Mason_CoreOnly] [ERROR] ExitCode={1}; Exception={2}" -f $errorStamp, $exitCode, $errorSummary
    ...
    Write-Host "Start Mason CoreOnly failed. See log: $logPath"
