...
$tmp   = Join-Path $rep ("http7001_{0}.jsonl" -f $stamp)
$zip   = Join-Path $rep ("http7001_{0}.zip"    -f $stamp)
...
try {
  ...
  Get-ChildItem -LiteralPath $rep -Filter 'http7001_*.zip' |
  ...
  try {
    if (Get-Command Write-Breadcrumb -ErrorAction SilentlyContinue) {
      Write-Breadcrumb -action 'http7001.rotate' -meta @{ size_mb = $sizeMB; zip = [IO.Path]::GetFileName($zip); keep = $Keep }
    }
  } catch {}
} catch {
  try { if (Test-Path $tmp) { Move-Item -LiteralPath $tmp -Destination ($zip + '.failedcopy.jsonl') -Force } } catch {}
}
