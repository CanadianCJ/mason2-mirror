...
$tmp   = Join-Path $rep ("alerts_{0}.jsonl" -f $stamp)
$zip   = Join-Path $rep ("alerts_{0}.zip"    -f $stamp)
...
try {
  ...
  Get-ChildItem -LiteralPath $rep -Filter 'alerts_*.zip' |
  ...
  try {
    if (Get-Command Write-Breadcrumb -ErrorAction SilentlyContinue) {
      Write-Breadcrumb -action 'alerts.rotate' -meta @{ size_mb = $sizeMB; zip = [IO.Path]::GetFileName($zip); keep = $Keep }
    }
  } catch {}
} catch {
