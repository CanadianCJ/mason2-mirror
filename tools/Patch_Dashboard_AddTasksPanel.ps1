...
# Adds a right-side, read-only "Scheduled Tasks" panel (mirrors Auto-Ops status).
...
# Helper snapshot function (inject once)
if($src -notmatch 'function\s+Get-M2TasksSnapshot'){
  $src += @"
...
# Runtime-injected panel (idempotent)
if($src -notmatch 'M2TasksPanelInjected'){
  $src += @"
...
Write-Host "Scheduled Tasks panel injected. Backup: $bak"
