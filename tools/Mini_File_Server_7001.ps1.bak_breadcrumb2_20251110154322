...
. (Join-Path $Base 'tools\common\Log.ps1') 2>$null
...
function _log($kv) {
  try {
  # Breadcrumb when server starts (PS5.1-safe)
   try {
     if(Get-Command Write-Breadcrumb -ErrorAction SilentlyContinue){
       if($kv.kind -eq 'server7001' -and $kv.event -eq 'started'){
         $pref = $null; if($kv.prefixes){ $pref = ($kv.prefixes -join ',') }
         Write-Breadcrumb -action 'server7001.started' -meta @{ prefixes = $pref }
       }
     }
   } catch {}
    $path = Join-Path $Base 'reports\http7001.jsonl'
    $lvl  = 'info'
    if($kv.err -or (($kv.status -as [int]) -ge 500)) { $lvl='error' }
    elseif(($kv.status -as [int]) -ge 400) { $lvl='warn' }
    if(Get-Command Write-Jsonl -ErrorAction SilentlyContinue){
      Write-Jsonl -Path $path -kv $kv -lvl $lvl
    } else {
      $kv['v']=1
      $kv['lvl']=$lvl
      $js = $kv | ConvertTo-Json -Compress
      Add-Content -LiteralPath $path -Encoding UTF8 -Value $js
    }
  } catch {}
}
# HTTP listener setup
    ...
    switch($path){
      ...
      '/metrics.json' {
        $obj = @{
          ...
          lat       = _tailJson (Join-Path $Base 'reports\latency_summary.jsonl')
        }
      ...
      '/alerts.json' {
        $p = Join-Path $Base 'reports\alerts.jsonl'
        $arr = @()
        if(Test-Path $p){
          $arr = Get-Content -LiteralPath $p -Tail 100 -ErrorAction SilentlyContinue | ForEach-Object { try{ $_|ConvertFrom-Json }catch{ $null } } | Where-Object { $_ }
        }
        $w = Write-RespText $res ($arr | ConvertTo-Json -Depth 6) 'application/json'
        _log @{ ts=(Get-Date).ToString('s'); kind='http7001'; corr=[Guid]::NewGuid().ToString('N'); ms=$null; method=$req.HttpMethod; path='/alerts.json'; status=$res.StatusCode; bytes=$w; ua=_redact($req.UserAgent); ip=([string]$req.RemoteEndPoint.Address) }
        continue
      }
      default {
        $res.StatusCode=404
